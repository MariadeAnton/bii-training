...
protected:
    ...
    void initialize(const char* v)          { tag = VAR_STRING; pt::initialize(PTR_TO_STRING(value.s), v); }
    void initialize(const string& v)        { tag = VAR_STRING; pt::initialize(PTR_TO_STRING(value.s), v); }
...
public:
    ...
    friend inline X* get(const tstrmap& m, const char* str)         { return (X*)pt::get((const strmap&)m, str); }
    friend inline void put(tstrmap& m, const string& str, X* obj)   { unknown* t = obj; pt::put(m, str, t); }
    X* operator[] (const char* str) const                           { return (X*)pt::get(*this, str); }
};
